...
def _topology_supported_accept(candidate, decision_cfg, primary_threshold, primary_floor=0.0):
    ...
    candidate_count_after_filter = int(
        candidate.get("candidate_count_after_filter", candidate.get("candidate_count_total", 0)) or 0
    )
    result = {
        "allowed": False,
    ...
    if float(candidate.get("topology_score", 0.0) or 0.0) < float(cfg.get("min_topology_score", 0.85)):
