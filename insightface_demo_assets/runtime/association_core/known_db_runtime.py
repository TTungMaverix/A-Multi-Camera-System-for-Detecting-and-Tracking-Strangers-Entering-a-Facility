...
def _write_manifest_rows(path: Path, rows):
...
def _discover_manifest_rows(project_root: Path, known_root: Path):
    rows = []
    image_paths = sorted(
    ...
    for image_path in image_paths:
        ...
        if parent not in id_by_dir:
            display = _safe_identity_name(parent.name if parent != known_root else image_path.stem)
...
def ensure_known_db_manifest_rows(project_root: Path, manifest_csv: Path, known_root: Path):
    rows = [row for row in _read_manifest_rows(manifest_csv) if row.get("gallery_rel_path")]
    auto_discovered = False
    if not rows and known_root.exists():
        rows = _discover_manifest_rows(project_root, known_root)
        if rows:
            _write_manifest_rows(manifest_csv, rows)
